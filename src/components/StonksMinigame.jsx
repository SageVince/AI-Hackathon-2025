...
const handle_collisions = () => { const pr = player_rect(); game.entities.forEach((e) => { const er = { x: e.x - e.w / 2, y: e.y - e.h / 2, w: e.w, h: e.h }; if (pr.x < er.x + er.w && pr.x + pr.w > er.x && pr.y < er.y + er.h && pr.y + pr.h > er.y) { if (e.kind === "coin") { game.coins += 3; e.y = HEIGHT + 100; } else if (e.kind === "bad") { const cost = 5; if (game.coins >= cost) game.coins -= cost; else { game.debt += cost - game.coins; game.coins = 0; } e.y = HEIGHT + 100; } else if (e.kind.startsWith("obst")) { game.game_over = true; } } }); game.entities = game.entities.filter(e => e.y < HEIGHT + 80); };
...
const draw = (ctx, theme) => {
    ...
    ctx.lineWidth = 2;
    for (const x of LANES_X) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, HEIGHT); ctx.stroke(); }
    for (const e of game.entities) { ctx.fillStyle = e.color; if (e.kind === 'coin') { ctx.beginPath(); ctx.arc(e.x, e.y, e.w / 2, 0, 2 * Math.PI); ctx.fill(); } else { ctx.fillRect(e.x - e.w / 2, e.y - e.h / 2, e.w, e.h); } }
    const pr = player_rect();
    ctx.fillStyle = theme.primary;
    ctx.fillRect(pr.x, pr.y, pr.w, pr.h);
    ctx.font = "bold 18px 'Segoe UI', 'Roboto', sans-serif";
    ctx.fillStyle = theme.accentPositive;
    ctx.fillText(`Coins: ${Math.floor(game.coins)}`, 16, 28);
    ctx.fillStyle = theme.primary;
    ctx.fillText(`Invest: ${Math.floor(game.investment)} (+${INV_APR * 100}% APR)`, 16, 52);
    ctx.fillStyle = theme.accentNegative;
    ctx.fillText(`Debt: ${Math.floor(game.debt)} (+${DEBT_APR * 100}% APR)`, 16, 76);
    ctx.font = "bold 20px 'Segoe UI', 'Roboto', sans-serif";
    ctx.fillStyle = theme.text;
    ctx.fillText(`Net Worth: ${Math.floor(game.net_worth)}`, 16, 108);
    ctx.font = "16px 'Segoe UI', 'Roboto', sans-serif";
    ctx.fillStyle = theme.textSecondary;
    ctx.fillText(`Speed: ${game.scroll_speed.toFixed(1)}`, 16, 134);
    ctx.fillText("I=Invest, L=Loan, ↑ Jump, ↓ Slide, P=Pause", 16, HEIGHT - 20);
    if (game.loan_boost_left > 0) { ctx.fillStyle = "#D97706"; ctx.font = "bold 18px 'Segoe UI', 'Roboto', sans-serif"; ctx.fillText(`Loan boost: ${game.loan_boost_left.toFixed(1)}s`, WIDTH - 220, 28); }
    if (game.show_tutorial) {
        ctx.fillStyle = `rgba(${theme.background === '#1a1a1d' ? '26,26,29,0.9' : '241,245,249,0.9'})`;
        ctx.fillRect(0, 0, WIDTH, HEIGHT);
        ctx.fillStyle = theme.primary; ctx.textAlign = 'center'; ctx.font = "bold 36px 'Segoe UI', 'Roboto', sans-serif";
        ctx.fillText("STONKS! The Minigame", WIDTH / 2, 120);
        ctx.textAlign = 'left'; ctx.font = "bold 20px 'Segoe UI', 'Roboto', sans-serif"; ctx.fillStyle = theme.text;
        ctx.fillText("GOAL: Maximize your Net Worth!", 150, 200);
        ctx.fillText("CONTROLS:", 150, 250);
        ctx.font = "18px 'Segoe UI', 'Roboto', sans-serif";
        ctx.fillText("← → : Switch Lanes", 200, 290);
        ctx.fillText("↑   : Jump (over low obstacles)", 200, 320);
        ctx.fillText("↓   : Slide (under tall obstacles)", 200, 350);
        ctx.fillText("I   : Invest $10 to grow your wealth", 200, 380);
        ctx.fillText("L   : Take a $50 Loan for a speed boost", 200, 410);
        ctx.font = "bold 24px 'Segoe UI', 'Roboto', sans-serif"; ctx.textAlign = 'center'; ctx.fillStyle = theme.primary;
        ctx.fillText("Press 'Enter' to Start!", WIDTH / 2, 500);
    }
    if (game.paused) { ctx.font = "bold 42px 'Segoe UI', 'Roboto', sans-serif"; ctx.fillStyle = theme.text; ctx.textAlign = "center"; ctx.fillText("PAUSED", WIDTH / 2, HEIGHT / 2 - 50); }
    ctx.textAlign = "left";
};
...
function StonksMinigame({ backToMenu, onGameEnd, theme }) {
  ...
  const gameLoop = useCallback(() => {
    ...
    const rawDt = now - game.last_time;
    game.last_time = now;
    const dt = rawDt * timeScale;

    if (!isGameOver && !game.show_tutorial && !game.paused && !game.game_over) {
    ...
    if (game.game_over && !isGameOver) {
      const score = Math.floor(game.net_worth);
      setFinalScore(score);
      setIsGameOver(true);
      // The onGameEnd call is removed from here.
    }
  ...
  const gameOverContent = (
    <div style={{ background: theme.cardBg, border: `1px solid ${theme.borderColor}`, borderRadius: '8px', padding: '40px', textAlign: 'center', color: theme.text }}>
        <h1 style={{ fontSize: '3rem', color: theme.accentNegative, marginBottom: '20px' }}>GAME OVER</h1>
        <h2 style={{ fontSize: '2rem', marginBottom: '30px' }}>{`Final Net Worth: $${finalScore.toLocaleString()}`}</h2>
        <button onClick={backToMenu} style={{ padding: '12px 25px', background: theme.primary, color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '1rem' }}>
            Back to Menu
        </button>
    </div>
  );
    ...
    <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
        {[1, 2, 4, 6, 8, 10].map(speed => (
            <button 
                ...
                style={{
                    padding: '8px 16px',
                    background: timeScale === speed ? theme.primary : theme.textSecondary,
                    color: timeScale === speed ? 'white' : theme.background,
                    border: 'none', 
                    borderRadius: '8px', 
                    cursor: 'pointer',
                    fontWeight: 'bold'
  ...
  return (
    <div style={{ background: theme.background, minHeight: 'calc(100vh - 60px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        {!isGameOver && <canvas ref={canvasRef} width={WIDTH} height={HEIGHT} style={{ background: theme.cardBg, border: `1px solid ${theme.borderColor}`, borderRadius: '8px', boxShadow: `0 4px 6px rgba(0,0,0,${theme.background === '#1a1a1d' ? 0.2 : 0.05})` }} />}
        {isGameOver && gameOverContent}
        {!isGameOver && (
            <>
                {speedControls}
                <button onClick={backToMenu} style={{ marginTop: '20px', padding: '12px 25px', background: theme.primary, color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '1rem' }}>
                    Quit Minigame
                </button>
            </>
        )}
